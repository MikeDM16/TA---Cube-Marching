...
void compute_gradient(float fieldCorner, float stepSize) {
    vec3 gradients[25][25][25];
    ...
    for (int i = 0; i < grads.length; i++) {
        ...
        gx = (density(px1) - density(px2))/stepSize;
        gy = (density(py1) - density(py2))/stepSize;
        gz = (density(pz1) - density(pz2))/stepSize;

        grads[i] = vec4(gx, gy, gz, 1);
    }
}

void emit_triangle(int x, int y, int z){
    vec4 a = vec4(vertices[z], 1);
    gl_Position = PVM * a;
    EmitVertex();

    vec4 b = vec4(vertices[y], 1);
    gl_Position = PVM * b;
    EmitVertex();

    vec4 c = vec4(vertices[x], 1);
    gl_Position = PVM * c;
    EmitVertex();

    vec3 normals[3];
    normals[0] = - 1 * normalize(vnormals[z]);
    normals[1] = - 1 * normalize(vnormals[y]);
    normals[2] = - 1 * normalize(vnormals[x]);
    
    DataOut.normal = (normals[0] + normals[1] + normals[2])/3;
}
